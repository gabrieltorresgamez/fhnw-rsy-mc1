...
p1 <- predict(IBCF1, as(df_test1, "realRatingMatrix"), type="topNList", n=30)
```
```{r}
#sample 2
IBCF2 <- Recommender(as(df_training2, "realRatingMatrix"), "IBCF",
                        param=list(normalize = NULL, method="cosine"))
p2 <- predict(IBCF2, as(df_test2, "realRatingMatrix"), type="topNList", n=30)
```
## Bestimme die Verteilung der Filme, welche bei IBCF für paarweise Ähnlichkeitsvergleiche verwendet werden
```{r}
#sample 1
df_movies1<- table(unlist(as(p1,"list"))) %>%
  as.data.frame()%>%
  rename(movieId = Var1)%>%
  arrange(desc(Freq))

ggplot(df_movies1,aes(Freq))+
  geom_histogram(bins =30)
```
```{r}
#sample 2
df_movies2<- table(unlist(as(p2,"list"))) %>%
  as.data.frame()%>%
  rename(movieId = Var1)%>%
  arrange(desc(Freq))


ggplot(df_movies2,aes(Freq))+
  geom_histogram(bins =30)
```
