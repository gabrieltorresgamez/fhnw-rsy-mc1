...
set.seed(100) 
#sample 2 
df_split2 <- initial_split(m_user_item_r1, prop = 0.80)
...
p1 <- predict(IBCF1, as(df_test1, "realRatingMatrix"), type = "topNList", n = 15)
```
...
IBCF2 <- Recommender(as(df_training2, "realRatingMatrix"), "IBCF",
                        param=list(normalize = NULL, method="cosine",k = 30))
p2 <- predict(IBCF2, as(df_test2, "realRatingMatrix"), type="topNList",n = 15)
```

## Bestimme die Verteilung der Filme, welche bei IBCF für paarweise Ähnlichkeitsvergleiche verwendet werden
```{r}
#sample 1
df_moviesIBCF1<- table(unlist(as(p1,"list"))) %>%
  as.data.frame()%>%
  rename(movieId = Var1)%>%
  arrange(desc(Freq))

ggplot(df_moviesIBCF1,aes(Freq))+
...
```{r}
#sample 2
df_moviesIBCF2<- table(unlist(as(p2,"list"))) %>%
  as.data.frame()%>%
  rename(movieId = Var1)%>%
  arrange(desc(Freq))


ggplot(df_moviesIBCF2,aes(Freq))+
  geom_histogram(bins =30)
```
...
sim_matrix1 <- as.matrix(similarity(as(df_training1, "realRatingMatrix"), method = "cosine", which = "items",))
...
# Sample 2
# calculate Similarity Matrix
sim_matrix2 <- as.matrix(similarity(as(df_training2, "realRatingMatrix"), method = "cosine", which = "items",))
# row index (movieId) to a separate column
...
```{r}
# sample 1
df_moviessmallICBF1 <- head(row_count1,30)%>%
  convert(int(movieId))
df_moviessmallICBF1<- left_join(df_moviessmallICBF1,norm_ratings) %>%
  group_by(movieId)%>%
  summarise(count = n(),
            mean = mean(z_rating))
```
...
```{r}
# sample 2
df_moviessmallICBF2 <- head(row_count2,30)%>%
  convert(int(movieId))
df_moviessmallICBF2 <- left_join(df_moviessmallICBF2 ,norm_ratings) %>%
  group_by(movieId)%>%
  summarise(count = n(),
            mean = mean(z_rating))
```
...
```{r}
# sample 1
ggplot(df_moviessmallICBF1,aes(count))+
  geom_histogram()
```

```{r}
# sample 2
ggplot(df_moviessmallICBF2,aes(count))+
  geom_histogram()
```
...
```{r}
# sample 1
ggplot(df_moviessmallICBF1,aes(mean))+
  geom_density()
```

```{r}
# sample 2
ggplot(df_moviessmallICBF2,aes(mean))+
  geom_density()
```
