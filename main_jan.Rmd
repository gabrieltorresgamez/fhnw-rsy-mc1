...
ratings1 %>%
  ...
  ungroup()%>%
  ggplot(., aes(rating,perc,color = factor(userId))) +
...
**Interpretation:**
Es gibt diverse Kunden, die nur Filme schauen, welche sie auch mögen.
...
**Beschreibung:**
Die Standardabweichungen der Bewertungen der User werden visualisiert. Dabei erkennt man, dass es eine Observation bei 0 gibt. Die meisten Observationen liegen zwischen 0.7 und 1.2

**Schlussfolgerung:**
Nur ein User hat alles gleiche Bewertungen. Wenn dieser User für einen Recommender mit standartisierten Daten verwendet wird ist es nicht mehr möglich eine Vorhersage zu machen.

Ansonsten ist der Median der Standartabweichungen leicht unter 1. Dies macht bei leicht linksschiefen Daten Sinn.
...
sparsity1 <- round(sum(is.na(user_item1)) / (dim(user_item1)[1] * (dim(user_item1)[2])),2)
```
...
sparsity2 <- round(sum(is.na(user_item2)) / (dim(user_item2)[1] * (dim(user_item2)[2])),2)
```

**Beschreibung:**
Die Sparsity beschreibt, wie hoch der Anteil der NA in den Daten ist. Im Datensatz 1 ist dieser Anteil `r sparsity1` % . Im Datensatz 2 ist die Sparsity `r sparsity2` %.

**Schlussfolgerung:**
Beide User-Item Matrizen sind etwa zu 98.6 % Sparse. Dies bedeutet, dass ein grosser Teil der Daten mit NA's besetzt ist. Für spätere Berechnungen ist es entscheidend, was mit diesen NA's gemacht wird.
...
**Beschreibung:**
Die Sparsities betragen `r sparsity_after_red1` % und `r sparsity_after_red2` %.
...
**Beschreibung:**
Die "Intersection over Union" ist `r intersection_over_union` gross.
...
Die Novelty ist ein Metrik, welches gut für Vergleiche geeignet ist. Es schwierig zu sagen, was eine Novelty von 2.5 bedeutet. Spannend zu sehen, ist das die Novelty unabhängig von der Länge der Top-N Liste ist.

# Wahl des optimalen Recommenders

## 10-Fache Krossvalidierung

Im folgenden wird eine 10-Fache Krossvalidierung. Damit eine Aussage gemacht werden kann, wurde eine minimale Anzahl von 3 Ratings bestimmt. Diese sollten jeweils in den Trainingsdaten vorhanden sein.
...
algorithms <- list(
  ...
  "Binary user-based CF" = list(name = "IBCF", param = list(k= 20,
                                                          method = "jaccard")),
  "Binary item-based CF" = list(name = "UBCF", param = list(nn=20,
                                                          method = "jaccard")),
...
* UBCF hat die höchste maximale Precision von 20 % und hat die grösste Standartabweichung bei den unterschiedlichen Folds.
* Die Precision sinkt bei der steigender Länge der Top-N Liste.
* Die Precision des Top Movie Recommenders verändert sich nicht jenach länge der Top-N Liste.
...
Die Modelle sind was die Precision anbelangt nicht alzu gut. Der Top-Movie Recommender ist gleich gut oder besser wie alle anderen Modelle. Eine mögliche Erklärung ist, dass das verwenden der standartisierten Ratings pro User nicht Sinnvoll waren.
...
Es war spannend zu sehen wie die unterschiedlichen Recommender Systeme aufgebaut sind. Für uns war die grosse Erkentnisse, dass die Evaluierung von Recommender Systemen nicht einfach ist. Um ein optimaler Recommender zu finden müsste man stärker mit Rückmeldung von Usern arbeiten. Zudem war es wahrscheinlich vom Vorgehen nicht optimal, die Ratings zu standartisieren.
