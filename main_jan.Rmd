...
```{r}
list_of_list <- getResults(results$`popular items`)
view(list_of_list)
```
...
novelty_values <-append(novelty_values,list(mean(unlist(nov_fold)),sd(unlist(nov_fold)),top,algorithm))
coverage_values <- append(coverage_values, list(mean(unlist(cov_fold)),sd(unlist(cov_fold)),top,algorithm))
}}
```
```{r}
novelty_values
