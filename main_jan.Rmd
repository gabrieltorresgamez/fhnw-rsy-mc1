...
**Beschreibung**
Die Sparsity beschreibt, wie hoch der Anteil der NA in den Daten ist. Im Datensatz 1 ist dieser Anteil `r sparsity1`. Im Datensatz 2 ist er `r sparsity2`
...
```{r}
#sample 1
scheme1 <-  evaluationScheme(as(as.matrix(m_user_item_r1),"realRatingMatrix"), method="cross-validation",k=10, given=5,goodRating  = 0)
```
```{r}
#sample 2
scheme2 <-  evaluationScheme(as(as.matrix(m_user_item_r2),"realRatingMatrix"), method="cross-validation",k=10, given=5,goodRating  = 0)
```
## Begründung der Modelle und Metriken

**Modelle:**
In dieser Aufgabe probieren wir die bereits kennengelernten Modelle aus. Für einen besseren Test, müssten die Hyperparameter der einzelnen Modelle zuerst Trainiert werden. Dies wird aus Zeitgründen nicht gemacht. Trotzdem werden ähnliche Parameter gleich verwendet. Somit werden zum Beispiel beim UBCF und IBCF Recommender jeweils 30 Neighbours verwendet.
...
plot <- (ggplot(pre_plot, aes(x=model, y= mean_cov)) + 
  ...
  geom_errorbar(aes(ymin=mean_cov- sd_cov, ymax=mean_cov+sd_cov), width=.2,
    ...
    subtitle = paste("Coverage Basis Modell: ",round(coverage_best_mean_model[[i]],2)),
    x = "Modell",
    y = "Coverage",
    caption = "Datensatz 1"))
   p[[i]] <- plot
...
plot <- (ggplot(pre_plot, aes(x=model, y= mean_novel)) + 
  ...
  geom_errorbar(aes(ymin=mean_novel- sd_novel, ymax=mean_novel+sd_novel), width=.2,
    ...
    labs(
    title = paste("Novelty von Top -",n_value," listen"),
    subtitle = paste("Novelty Basis Modell: ",round(novelty_best_mean_model2[[i]],2)),
    x = "Modell",
    y = "Novelty",
    caption = "Datensatz 2"))
   p2[[i]] <- plot
...
Das IBCF Modell hat zwei Hyperparameter. Die Anzahl Nachbarn und ob die NA auf 0 gesetzt werden. Die Hyperparameter werden mit der Top-N Liste von 30 und der Precision als Metrik gemacht. Die Precision wurde gewählt, weil sie als wichtigste Metrik deffiniert wurde. Es wäre auch möglich eine Kombination von Metriken zu wählen.
...
for (neighbours in (1:20)*2){
  for (na_as_zero in c(TRUE,FALSE)){
    ...
    results_best_model <- evaluate(scheme1, algorithm_test, type="topNList" , n= 30)
...
# predict model
pred_model <- predict(trained_model, getData(scheme1,"unknown",run = n_split), type = "topNList", n = 30)
nov_fold <- append(nov_fold,novelty(pred_model,log_popularity1,top,40))
cov_fold <- append(cov_fold,coverage(pred_model))
}
nov_fold1 <- mean(unlist(nov_fold))
cov_fold1<- mean(unlist(cov_fold))
```
...
for (neighbours in (1:20)*2){
  for (na_as_zero in c(TRUE,FALSE)){
    algorithm_test <- list("item-based CF" = list(name="IBCF", param=list(method="Cosine",k=neighbours,na_as_zero = na_as_zero)))
    results_best_model <- evaluate(scheme2, algorithm_test, type="topNList" , n= 30)
    mean_precision <- avg(results_best_model)[[1]][[6]]
    if (best_precision < mean_precision) {
...
```{r}
mean_nov2 <- mean(unlist(nov_fold))
mean_cov2 <- mean(unlist(cov_fold))
```

**Beschreibung**
Beim Datensatz 1 erzielt der beste Algorithmus basierend auf der Precision, eine Precision von `r round(best_precision,2)` eine Novelty von `r round(mean_nov1,2)` und eine Coverage von `r round(mean_cov1,2)`.

Beim Datensatz 2 erzielt der beste Algorithmus basierend auf der Precision, eine Precision von `r round(best_precision2,2)` eine Novelty von `r round(mean_nov2,2)` und eine Coverage von `r round(mean_cov2,2)`.
...
TODO 
Wieso Testkunden Random gewählt werden:
