...
### Beschreibung
Hier wird visualisiert, wie oft Filme bei IBCF und UBCF empfohlen werden. Auf der x-Achse befindet sich die Anzahl Empfehlungen pro Film und auf der y-Achse wieviele Filme gleich oft empfohlen wurden.

### Schlussfolgerung
Der ICBF Recommender empfiehlt mehr unterschiedliche Filme. Der UCBF Recommender empfiehlt bis zu 24 mal den gleichen Film.

# Analyse Top-N Listen - Ratings
Funktion um die Überschneidungen zu berechnen.
```{r}
intersection_Top15 <- function(prediction1, prediction2,Model1, Model2,Datensatz) {
...
## IBCF (Na_as_zeros = True) vs IBCF (Na_as_zeros = True)
...
for (singular_value in c(10,20,30,40,50)){
  print(intersection_Top15(pIBCF1,SVD_pred(training1,test1,singular_value),paste("SVD mit ",singular_value, " Singulärwerten"),"IBCF","1"))
}
```
...
```{r}
novelty <- function(model,log_popularity,topn){
predTopN <- as.data.frame(as(model, "matrix"))
predTopN$user <- rownames(predTopN)
predTopN <- pivot_longer(predTopN, cols = -c(user), values_drop_na = TRUE) %>%
  rename(movieId = name)
...
return(-sum(pred_with_log$log_pop)/400/topn)
}
```
