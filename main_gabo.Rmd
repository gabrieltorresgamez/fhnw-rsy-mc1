...
calculate_cos <- function(arr1, arr2) {
  ...
  if (sum(vals) != 0) {
    ab <- sum(arr1 * arr2)
    norma <- sqrt(sum(arr1 * arr1))
    normb <- sqrt(sum(arr2 * arr2))
    cos_sim <- ab / (norma + normb)
    return(cos_sim)
  }
  # If not, return NA
  return(NA)
...
```{r}
getCorrelationMatrixMatrix <- function(data) {
  data[is.na(data)] <- 0
  data <- t(data)
  AAT <- data %*% t(data)
...
getCorrelationMatrixMatrix(item_user_random_100_recommenderlab)[1:6,1:6]
```

```{r}
matrix <- matrix(c(2, 4, 4, 8), nrow = 2, ncol = 2)
matrix
```
