...
# Gesamthaft
summary(ratings)
ggplot(ratings, aes(x = rating)) + geom_boxplot() + theme(legend.position="none")
hist(ratings$rating, main = "Total")

# Nach Genres
genres_sep_ratings <- genres_sep %>%
  right_join(ratings, "movieId")
ggplot(genres_sep_ratings, aes(y = genre, x = rating, fill = genre)) + geom_boxplot() + theme(legend.position="none")
```
