...
```
# Implementierung
## Berechnung der Ähnlichkeitsmatrix per jaccard similarity
```{r}
...
getCorrelationMatrixJaccard <- function(data) {
  ...
  for (i in movies) {
    ...
    for (j in movies) {
      ...
      # If jaccard similarity was already calculated, skip, else continue
      if (i_counter <= j_counter) {
        # calculate jac sim
        row_j <- data %>% filter(movieId == j)
        sim <- calculate_jaccard(row_i, row_j)
        # set jac sim in sim matrix
...
## Berechnung der Ähnlichkeitsmatrix per cosinus similarity
```{r}
...
getCorrelationMatrixCos <- function(data) {
  # Get array with movieId's
  movies <- as.character(data$movieId)
...
getCorrelationMatrixJaccard(item_user_random_100_bool)
```
